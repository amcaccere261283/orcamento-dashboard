...
test('renderControles inclui o toggle de congelamento e os status spans no MESMO bloco do seletor de semana', () => {
  const html = renderControles({ semanas: SEMANAS, semanaIdx: 0 });
  // Um SÓ <div class="controles-consolidado"> -- prova que não sobrou um
  // segundo bloco em outro lugar.
...
test('toggle-congelamento nasce desabilitado, conforme a interface de carga inicial', () => {
  const html = renderControles({ semanas: SEMANAS, semanaIdx: 0 });
  assert.match(html, /<input type="checkbox" id="toggle-congelamento" disabled>/);
});
