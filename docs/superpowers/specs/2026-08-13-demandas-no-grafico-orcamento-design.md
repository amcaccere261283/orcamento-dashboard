...
Demandas, `tools/semanal/compute-demandas.js`): a série `chegadas` conta, por mês, quantos
itens de demanda (furos de sondagem + ensaios de laboratório) chegaram — um FLUXO que já
acumula por soma corrida, exatamente a mecânica que o Gráfico do orçamento já usa para
Previsto/Realizado/Tendência. É essa série — só ela, não `pendentes`/`canceladas`/
`relatorioConcluido`/`sondagemRealizada` — que este trabalho traz para o orçamento.
...
`parseAvancos`/`parseLab` (movidos ou reexportados de `tools/semanal/` — não duplicar a
leitura de CSV/JSON), `redirecionarSupsDesconhecidos(furos, registros)` e
...
semanal usa) nem dos eventos crus de `porRegistroEventos` — precisa de "chegadas por mês,
por (sup, tipologia)", já bucketizado. **Diferença importante em relação à semanal:** lá o
bucketing por mês acontece no CLIENTE (`contarEventosNoIntervalo`, porque a página tem
controles de período/semana que mudam em runtime); aqui o Gráfico do orçamento só tem 12
meses fixos do ano do build (mesmos `periodos` que `parseMatriz`/`renderDashboard` já usam
para todo o resto), então o bucketing pode — e deve — acontecer no BUILD, em Node, evitando
duplicar lógica de data no cliente e evitando ter que embutir o ano/objetos `Date` no HTML
publicado (hoje o cliente só tem `MESES_ABREVIADOS`, rótulos fixos, e `window.__VIGENTE_IDX__`
— nenhuma data real).
...
// fimDoMes (já definidas acima no módulo); roda sobre os MESMOS furos/
// ensaiosLab, então tem que ver exatamente os mesmos eventos que
// porRegistroEventos[chave].chegada já contaria -- validar com um teste
// que soma este resultado e compara contra a soma de chegada.length por
// chave.
...
como `{ chave: [12 inteiros] }`. Nova função, ao lado de `mensalBruto`/`calcularAcumulado`
em `construirPainelGraficoHtml`, só SOMA os arrays das chaves que passam no filtro atual:
...
mudança — Demandas é fluxo, não estoque, mesma premissa que `acumularSerie` de
`render-aba-demandas.js` já documenta).
...
- **`chegadasMensaisPorRegistro` (build, Node)**: soma de todas as chaves bate com a soma de
  `chegada.length` que `porRegistroEventos` já produziria para os mesmos furos/ensaios
  (prova de que o bucketing por registro não perde nem duplica evento); evento fora da
  janela dos 12 `periodos` não entra em nenhum mês (mesmo comportamento de `indiceDoMes`
  devolvendo -1); chave sem nenhum evento não aparece no objeto (ou aparece com 12 zeros —
  decidir na implementação, documentar o escolhido).
...
- **`dadosPorSerie` com Demandas**: dimensão Volume inclui a 5ª entrada com `mensal`/
  `acumulado` corretos; as outras 4 dimensões NÃO incluem Demandas (array de 4 entradas,
  não 5).
